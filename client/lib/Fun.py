...
#设置控件的文本(Label、Button、Entry、Text)。参数1：界面类名, 参数2:控件名称，参数3:文本内容。
...
#获取控件的文本(Label、Button、Entry、Text)。参数1：界面类名, 参数2:控件名称。
...
#设置控件的绝对或相对位置。参数1：控件 ,参数2：x位置，参数3:y位置 ,参数4：高度，参数5:宽度 。
...
#画板动作处理函数
def DoCanvasRecord(drawCanvas,type,x,y,x2,y2,fillcolor,outlinecolor,fillwidth,dash1=0,dash2=0,text=''):
    if  drawCanvas != None:
        if type == 'line' or type == 'pen'  :
            ...
            drawCanvas.create_text(x, y,fill=fillcolor,text=text,tag='text')
#读取画板动作记录
def LoadCanvasRecord(uiName):
    drawCanvas = None
    canvasFile = os.getcwd() + "\\" + uiName + ".cav"
        ...
        while True:
            ...
            if text.find('Canvas:') >= 0:
                ...
                if drawCanvas != None:
                    ...
                    text = splitArray[10]
                    DoCanvasRecord(drawCanvas,type,x,y,x2,y2,fill,outline,width,dashx,dashy,text)
                continue
        f.close()  
...
class WindowDraggable():
    ...
    def LeaveDragBorder_BR(self,event):
        ...
        self.bottom_drag.place_forget() 
        self.widget.configure(cursor='arrow')
#使用TKinter方式设置窗口圆角, 支持跨平台。参数1:Canvas控件,参数2:x位置,参数3:y位置,参数4：高度,参数5:宽度,参数6:圆角半径。
